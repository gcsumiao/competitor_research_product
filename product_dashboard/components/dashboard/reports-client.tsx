...
export function ReportsClient({ data, reports }: { data: DashboardData; reports: ReportFile[] }) {
  ...
  const availableReports = selectedCategoryId
    ? getReportsForCategory(selectedCategoryId, reports)
    : reports
              ...
              <tbody>
                {availableReports.length ? (
                  availableReports.map((report) => (
                    <tr
                      key={`${report.source}:${report.relativePath}`}
                    ...
                    >
                      ...
                      <td className="py-3 px-2 text-xs text-right">
                        <a
                          ...
                          href={`/api/report?source=${encodeURIComponent(report.source)}&file=${encodeURIComponent(report.relativePath)}`}
                        >
